...
void grid::reconstruct() {
  if (opts().use_legacy_reconstruct)
    reconstruct_legacy();
  else
    reconstruct_vc();
}

void grid::reconstruct_legacy() {
...
void grid::reconstruct_vc() {
	...
	for (integer field = 0; field != opts().n_fields; ++field) {
        ...
        for (integer iii = H_NX * H_NX; iii != H_N3 - H_NX * H_NX; iii += m2m_vector::size())  {
            ...
            // loop 2
			auto uf_avg = average(
                m2m_vector(Vfield.data() + iii + H_DNX), m2m_vector(Vfield.data() + iii));
